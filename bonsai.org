...
We default to using ~bsbca~ and estimate some descriptive statistics
in ~summarize~. 

# Verb ~bonsai~ is ambivalent, and
# when used as a dyad benchmarks two sentences and compares their mean
# execution times via bootstrap-t and reports some descriptive
# statistics for each. When used as a monad, it just outputs the
